// src/App.jsx — 3 providers: MoviesDrive + MultiStream + MyFlixBD
...
// ✅ Correct value names from vega-providers/manifest.json
// myflixbd = custom provider (our own)
const PROVIDERS = [
  { value: 'drive',     display_name: 'MoviesDrive', type: 'global', icon: '' },
  { value: 'autoEmbed', display_name: 'MultiStream', type: 'global', icon: '' },
  { value: 'myflixbd',  display_name: 'MyFlixBD',   type: 'india',  icon: '' },
]
...
function Shell() {
  ...
  useEffect(() => { providerStorage.setInstalled(PROVIDERS) }, [])
  ...
  useEffect(() => {
    ...
    const fn = () => { goBack(); window.history.pushState({ p: page }, '') }
  ...
  return (
    <div className="app">
      <div className="ambient-orb orb-1" /><div className="ambient-orb orb-2" />
      {showNav && <Navbar page={page} navigate={navigate} />}
      {showNav && (
        <nav className="bottom-nav">
          {[{id:'home',Icon:Icons.Home},{id:'search',Icon:Icons.Search},{id:'watchlist',Icon:Icons.Heart},{id:'history',Icon:Icons.Clock}]
            .map(({id,Icon}) => (
