...
function streamMode(stream) {
  ...
  if (t === 'hls' || t === 'm3u8' || url.includes('.m3u8') || url.includes('/manifest')) return 'hls'
  return 'hls'
}
...
export default function PlayerPage({ params, navigate }) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        console.log('[Player] streams:', valid.length, valid.map(s => ({ server: s.server, type: s.type, url: s.link?.slice(0, 80) })))
        if (valid.length) { setStreams(valid); setSelected(valid[0]) }
        else setError('No streams found. Try another provider or server.')
      } catch (e) {
  ...
  useEffect(() => {
    ...
    const mode  = streamMode(selected)
    if (hlsRef.current) { hlsRef.current.destroy(); hlsRef.current = null }
    setQualities([]); setAudioTracks([]); setBuffering(false)
    console.log('[Player] loading:', url.slice(0, 80), '| mode:', mode)
    ...
    if (mode === 'hls') {
      if (Hls.isSupported()) {
        ...
        hls.on(Hls.Events.ERROR, (_, d) => {
          console.error('[HLS error]', d.type, d.details, d.fatal)
          if (d.fatal) {
            if (hlsRef.current) { hlsRef.current.destroy(); hlsRef.current = null }
            video.src = url
        ...
        setError('HLS not supported. Try another browser.')
      }
    } else {
      video.src = url; video.play().catch(() => {})
    }
  ...
  useEffect(() => {
    ...
    const onErr   = () => setError('Playback error. Try another server.')
    ...
    v.addEventListener('canplay', onCan); v.addEventListener('error', onErr)
    ...
    return () => {
      ...
      v.removeEventListener('canplay', onCan); v.removeEventListener('error', onErr)
  ...
  const tryNextServer = () => {
    ...
    const next = streams[(streams.indexOf(selected) + 1) % streams.length]
    if (next !== selected) selectServer(next)
  }
    ...
    <div ref={containerRef} className="player-page"
      ...
      {error && !loading && (
        <div className="player-center-overlay">
          ...
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', justifyContent: 'center' }}>
            {streams.length > 1 && <button className="btn btn-glass" onClick={tryNextServer}><Icons.Refresh /> Try Next Server</button>}
      ...
      <div className={`player-controls-overlay ${showControls ? 'visible' : ''}`}>
        <div className="player-top-bar">
          ...
          <button className="player-icon-btn" style={showServers ? { background: 'rgba(26,111,255,0.5)' } : {}} onClick={() => { setShowServers(s => !s); setShowSettings(false) }}><Icons.Globe /></button>
          <button className="player-icon-btn" style={showSettings ? { background: 'rgba(26,111,255,0.5)' } : {}} onClick={() => { setShowSettings(s => !s); setShowServers(false) }}><Icons.Settings /></button>
        </div>
      ...
      {showServers && streams.length > 0 && (
        <div className="player-panel">
          <div className="player-panel-title"><Icons.Globe /> Servers ({streams.length})<button className="player-panel-close" onClick={() => setShowServers(false)}><Icons.X /></button></div>
      ...
      {showSettings && (
        <div className="player-panel">
          <div className="player-panel-title"><Icons.Settings /> Settings<button className="player-panel-close" onClick={() => setShowSettings(false)}><Icons.X /></button></div>
          {qualities.length > 0 || audioTracks.length > 0 ? (
            <>
              ...
              <div className="player-panel-list">
                {settingsTab === 'quality' && qualities.map(q => <button key={q.id} className={`player-panel-item ${curQuality === q.id ? 'active' : ''}`} onClick={() => changeQuality(q.id)}>{q.label} {curQuality === q.id && <Icons.Check />}</button>)}
                {settingsTab === 'audio'   && audioTracks.map(t => <button key={t.id} className={`player-panel-item ${curAudio === t.id ? 'active' : ''}`} onClick={() => changeAudio(t.id)}>{t.label} {curAudio === t.id && <Icons.Check />}</button>)}
              </div>
            </>
          ) : (
            <div style={{ padding: 24, color: '#555', fontSize: 13, textAlign: 'center' }}>Quality options appear after stream loads</div>
          )}
        </div>
      )}
    </div>
