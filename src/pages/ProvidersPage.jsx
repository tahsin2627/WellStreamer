...
export default function ProvidersPage() {
  ...
  useEffect(() => {
    fetchManifest().then(data => setManifest(data || [])).catch(() => {}).finally(() => setLoading(false))
  }, [])
    ...
    <div className="page fade-in">
      <div className="page-header">
        <div>
          ...
          <p className="page-sub">{installed.length} installed · {manifest.length} total · auto-updates from Vega's GitHub</p>
        </div>
      ...
      <div className="provider-controls">
        <div className="tabs">
          <button className={`tab ${tab === 'available' ? 'active' : ''}`} onClick={() => setTab('available')}>Available ({manifest.filter(p => !isInstalled(p.value)).length})</button>
          <button className={`tab ${tab === 'installed' ? 'active' : ''}`} onClick={() => setTab('installed')}>Installed ({installed.length})</button>
        </div>
        <input className="field-input" style={{ maxWidth: 240, padding: '8px 14px' }} placeholder="Filter providers…" value={search} onChange={e => setSearch(e.target.value)} />
      </div>
      ...
      <div className="providers-grid">
        {filtered.map(p => {
          const busy = isInstalling[p.value], fb = feedback[p.value], inst = isInstalled(p.value)
          return (
            <div key={p.value} className="provider-card glass2">
              <div className="provider-icon">
                {p.icon ? <img src={p.icon} alt="" onError={e => { e.target.style.display = 'none'; e.target.nextSibling.style.display = 'flex' }} /> : null}
                <span style={{ display: p.icon ? 'none' : 'flex', fontSize: 22, alignItems: 'center', justifyContent: 'center', width: '100%', height: '100%' }}>{FLAG[p.type] || '🌐'}</span>
              </div>
              ...
              <div className="provider-action">
                ...
                {!fb && inst  && <button className="btn btn-glass btn-sm" onClick={() => handleUninstall(p.value)}><Icons.Trash /></button>}
                {!fb && !inst && <button className="btn btn-primary btn-sm" onClick={() => handleInstall(p)} disabled={busy}>{busy ? <span className="btn-spinner" /> : <Icons.Download />}</button>}
              </div>
            </div>
          )
        })}
